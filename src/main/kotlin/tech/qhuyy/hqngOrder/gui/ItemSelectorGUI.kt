...
class ItemSelectorGUI(private val plugin: HqngOrder) {
    ...
    private fun reopen(player: Player) {
        ...
        if (!handItem.type.isAir) {
            gui.setItem(
                4, ItemBuilder.from(handItem.clone())
                    .name(
                        formatter.deserializeKey(
                            "gui.item-selector.hand-item-name",
                            TagResolver.resolver(
                                "item_name",
                                Tag.inserting(net.kyori.adventure.text.Component.text(handItem.type.name))
                            )
                        )
                    )
                    .lore(formatter.deserializeKey("gui.item-selector.hand-item-lore"))
                    .asGuiItem { event ->
        ...
        val slots = mapOf(
            46 to "filter_all", 47 to "filter_blocks", 48 to "filter_tools",
        ...
        val materials = when (currentCategory) {
            ...
            "misc" -> allMaterials.filter { m ->
                val isBlock = m.isBlock
                val isTool = m.name.endsWith("_AXE") || m.name.endsWith("_PICKAXE") ||
                        m.name.endsWith("_SHOVEL") || m.name.endsWith("_HOE") ||
                        m.name.contains("FISHING") || m.name.contains("SHEARS") || m.name.contains("FLINT") || m.name.endsWith(
                    "_SWORD"
                )
        ...
        for (mat in materials) {
            val item = ItemBuilder.from(mat)
                .name(
                    formatter.deserializeKey(
                        "gui.item-selector.material-name",
                        TagResolver.resolver(
                            "material_name",
                            Tag.inserting(net.kyori.adventure.text.Component.text(mat.name))
                        )
                    )
                )
                .asGuiItem { event ->
